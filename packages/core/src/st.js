...
// should hopefully catch any bad typings

export const makeStComponentCreator =
    ({ styleManager }) =>
    () => {
        return function st(options) {
            const { el, defaultAttrs = [], defaultProps = [], className = [], forwardAttrs = [], forwardCss = [], css = [], Component } = options;
            ...
            const St = function St({ children, as, attrs, css, className, ...props }) {
                const { bpIndex, breakpoints } = useSt();
                ...
                // should this use cloneElement?
                return Component({ C: as || el, attrs: { children, ...mergedAttrs }, ...mergedProps });
            };
